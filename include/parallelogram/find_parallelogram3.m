function [PONTOS, alpha] = find_parallelogram3(img_original, dT, rho, theta, pic)

%
% dada a imagem original "img_original", o step angular "dT", e a lista de
% "rho" e "theta" com valores de pico "pic", a rotina find_parallelogram3
% procura paralelogramos genericos e retorna os vertices encontrados
%
alpha=[];
...
ang = 180/pi*theta';
L_theta_intra = max(6,3.1*180*dT/pi);  %limiar do angulo para o mesmo par
L_pico = .5; %limiar de tamanho dos lados dos retangulos (em %)
...
for index1=1:length(rho);
    for index2=(index1+1):length(rho);
        ...
        if (diferenca_theta<=L_theta_intra);
            ...
            if (diferenca_pico<=(pic(index1)+ pic(index2))*L_pico/2),
...
if (length(ang)>1)
    for n1=1:length(ang),
        for n2=(n1+1):length(ang),
            %
            %  mudado aqui pra pegar angulos qualquer
            %
            diferenca_ang = ang(n2)-ang(n1);angulo=min(diferenca_ang,180-diferenca_ang); 
            if (diferenca_ang>=MIN_internal),
                erro=[drho(n1) drho(n2); pic(n2)*sin(angulo*pi/180) pic(n1)*sin(angulo*pi/180)];  
                erro=abs(diff(erro)./(erro(1,:)+1e-10));disp(sprintf('erro max: %f',max(erro)));
                if max(erro)<=L_comp,
                    indice = indice+1;
%                    rect(:,:,indice)=[theta1(n1) theta2(n1) theta1(n2) theta2(n2);rho1(n1) rho2(n1) rho1(n2) rho2(n2) ]; % armazena rhos e thetas pros paralelogramos
                    rect(:,:,indice)=[theta(n1) theta(n1) theta(n2) theta(n2);rho1(n1) rho2(n1) rho1(n2) rho2(n2) ]; % armazena rhos e thetas pros paralelogramos
                    alpha(indice)=angulo;
                end
            end;
        end;
...
if ~isempty(rect),
    ...
    for i=1:ss(3),
        ...
        x=[P1 P2 P3 P4 P1]; 
        PONTOS(:,:,i)=x;
     %   line(PONTOS(2,:,i),PONTOS(1,:,i),'LineWidth',2,'Color','r'); axis ij; hold on;
    end,
end;
